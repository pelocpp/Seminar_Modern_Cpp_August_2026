...
namespace Auto_Examples {
    ...
    static void test_01() {
        ...
        auto n = std::string("ABC");   // Type Deduction
    ...
    // Dictionary:  Schlüssel (Key) und Wert (Value)
    ...
    static void test_03() {
        ...
        // Paare (Schlüssel, Wert)  :  std::pair<int, std::string>
        std::unordered_map<int, std::string> anotherMap{ { 1, "Hello"  } };

        std::unordered_map<int, std::string>::iterator it = anotherMap.begin();  // Position

        // So:  entry1 eine Kopie: 
        // Anders: entry1 ist ein Alias vom Original: Da muss der Typ EXAKT sein.
       // std::pair<int, std::string>& entry1 = *it;  // Why this line DOES NOT compile ???
        // type of key must be 'const int'
    ...
    static auto sum(double f1, double  f2)
    {
        return f1 + f2;  // ES GIBT in der CPU KEINE short-Arithmetik
    }

    template <typename T1, typename T2>
    static auto tueWas (bool flag, T1 f, T2 d) -> decltype (f + d)
    {
        if (flag) {
            return d;
        }
    ...
    void test_auto_seminar()
    {
        auto result = tueWas(false, 123, 123ll);     
    }
    ...
    public:
        ...
        // getter
        std::string getNameClassic() { return m_name; }
    ...
    static void test_06() {
        ...
        std::string hansName("Hans");

        Person hans(hansName);

        const std::string& name1 = hans.getName();   // Are we happy ??? KEINE Kopie
        // name1[0] = '!'; ERROR
        std::println("Message: {}", name1);

        const auto& name2 = hans.getName();   // Are we happy ??? KEINE Kopie
        //name2[0] = '!'; // ERROR
