...
static void lambda_02_01()
{
    ...
    std::for_each(
        vec.begin(),
        vec.end(),
        // will ich header: ALS Kopie, oder: Durch eine Referenz verwenden ??
        // Kopie: =
        // Referenz: &
        [=] (int n) {
...
static void lambda_02_02()
{
    ...
    auto l1 = [&n, m, &k] () { std::println("Copy:      {} {}", n, m, k); };
...
void test()
{
   ...
   // auto func = [](std::string par) { std::print("{}", par); };
...
static void lambda_06()
{
    ...
    auto increment = [value = value1] () mutable {
...
{
    auto lambdaOne = [](int a) { std::println("One: {}", a); };
    auto lambdaTwo = [](int b) { std::println("Two: {}", b); };

    std::vector<std::function < void (int) > > myLambdas;
...
static auto helper_b() {
    ...
    int n{ 1 };  // HEAP   // Closure:  JavaScript
    int m{ 2 };  // HEAP
